...
- Every Mobile checkmark is exercised by `e2e/mobile-tools.spec.ts` through the independent `npm run test:browser-mobile` production build/preview command in explicit touch Chromium at **375 × 812**. It asserts the exact viewport, meaningful render, no framework overlay or console errors, no document-level horizontal overflow, in-viewport primary-control geometry, and a keyboard/text interaction for all 41 public tool/chat routes. Each of the six file routes also uses a valid local PNG fixture and waits for its route-specific result UI.
- The Privacy checkmarks for six file tools come from asynchronous completed-workflow tests: local result plus actual download where the tool has downloadable output, with `fetch` and `XMLHttpRequest.send` observed from selection through completion. The browser contract separately rejects any external request containing the fixture filename or fixture bytes; benign runtime/analytics requests and background-model GET assets carry no user payload and are explicitly not treated as uploads. ImageConverter's only re-fetch is its local `data:` source with no request body.
